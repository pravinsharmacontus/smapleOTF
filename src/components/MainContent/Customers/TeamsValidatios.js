...
export function OwnerValidation( data = "") {
  ...
  const inviteMemberList = state?.inviteMemberListReducer?.inviteMember
...
export const submitTeams = (data, hostDetails, currentOrganisation) => {
  const inviteMember = {
    ...hostDetails,
  ...
  setTimeout(() => {
    store.dispatch(inviteMemberListAction(currentOrganisation));
